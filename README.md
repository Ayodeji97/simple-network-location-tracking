...
- Tells the difference between "the phone says it's connected" and "the internet
  actually works" by pinging Google's DNS server (`8.8.8.8`) over ICMP.
- Records each outage as a tidy entry: start time + place, end time + place, total
  duration, and whether the server was reachable.
- Reverse-geocodes coordinates into human-readable addresses where it can.
...
| **Coroutines core** | `NetworkWithLocationTracker` — `flatMapLatest` from connectivity into pinging, `distinctUntilChanged`, `catch`, `flowOn`, status published instantly via `StateFlow` while the location fix resolves separately, and a `SharedFlow` of tracking events |
...
and the data mapper — is covered by **20 fast JVM unit tests**. No device or
...
`BUILD SUCCESSFUL` means all 20 are green. Helpful variations:
...
- **`NetworkWithLocationTrackerTest`** (9) — the heart of it: connect/disconnect
  transitions, starting and completing outages, server-reachability, error paths.
...
- **`NetworkOutageMapperTest`** (2) — entity ⇄ domain round-trips.
